...
The repo-root markdown files are canonical. The Astro/Starlight site at
`docs/` mirrors them via `docs/scripts/preprocess.py`. The knowledge
layer at `knowledge/` is also canonical and is mirrored by the same
preprocess pass.

- **Canonical at the repo root:** `README.md`, `INSTALL.md`,
  `DESIGN.md`, `CONTRIBUTING.md`, `CHANGELOG.md`, `docs/roadmap.md`,
  and every file under `knowledge/`.
- **Site mirrors only:** anything under `docs/src/content/docs/`
  generated by `docs/scripts/preprocess.py` (currently `concepts/design.md`,
  `roadmap.md`, and the `concepts/knowledge/` tree).
- **When updating a top-level doc:** edit the root file, then run
  `just preprocess` (or let CI do it). Don't dual-author — changes
  made directly in the generated mirror will be overwritten.
- **When adding a new top-level doc:** if the preprocess pass doesn't
  pick it up automatically, file a quick follow-up PR to wire it in.

The hand-authored site pages (the landing page, how-to guides, the
reference pages, and concept pages that are not knowledge mirrors)
live under `docs/src/content/docs/` and are edited directly. They are
not generated.
