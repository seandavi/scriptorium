...
- **Identifier-driven citations are non-negotiable.** Manubot's `[@doi:...]` syntax is the right unit. Scriptorium's `citation-audit` should assume an identifier-resolved bibliography (CSL-JSON or BibTeX with verified DOIs) and refuse to operate on a bibliography with bare-text citations.
